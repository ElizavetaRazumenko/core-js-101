...
function getComposition(f, g) {
  ...
  return function argumented(...args) {
    return f(g(...args));
  };
}
...
function getPolynom(...args) {
  // throw new Error('Not implemented');
  return args.length === 0 ? null : function getResult(num) {
    let power = args.length - 1; let
      func = 0;
    for (let i = 0; i < args.length; i += 1, power -= 1) {
      func += args[i] * num ** power;
    }
    return func;
  };
}
...
function logger(func, logFunc) {
  // throw new Error('Not implemented');
  return (...args) => {
    const argsToStting = args.map((item) => JSON.stringify(item)).join(',');
    const startStr = `${func.name}(${argsToStting}) starts`;
    const endStr = `${func.name}(${argsToStting}) ends`;
    logFunc(startStr);
    const res = func(...args);
    logFunc(endStr);
    return res;
